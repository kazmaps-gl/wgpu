...
impl crate::Error {
    ...
    /// that still rejects with `TypeError`/`OperationError`) is reported as internal,
    /// rather than being misfiled as a validation error.
    fn from_pipeline_error(value: JsValue) -> Self {
        let source = Box::<dyn core::error::Error + Send + Sync>::from("<WebGPU Error>");
        let Some(error) = value.dyn_ref::<ext_bindings::GpuPipelineError>() else {
